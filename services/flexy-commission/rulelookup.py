...
class RuleLookup:
    ...
    def look(self):
        ...
        len_header_hash = {}
        filtered_rules = (h for h in hashes if h in config.FlexyRuleStorage.rule_definitions)

        for h in filtered_rules:
           len_header_hash.update({len(config.FlexyRuleStorage.rule_definitions[h]["header"]): h})

        print(max(len_header_hash.keys()))
        max_len_hash = len_header_hash[max(len_header_hash.keys())]        

        print(max_len_hash)

        rules = config.FlexyRuleStorage.rule_definitions[max_len_hash]
